...
private:
    /* DSP objects */
    DelayLine fDelayLineL;   ///< Delay line for the left channel.
    DelayLine fDelayLineR;   ///< Delay line for the right channel.
    LFO fLfoL;               ///< LFO modulating the left channel delay.
    LFO fLfoR;               ///< LFO modulating the right channel delay.

    /**
     * @brief Processes a single sample for the chosen channel.
    ...
    /* Local storage */
    float fParams[NUM_PARAMS] {};   ///< Parameter values. @see paramInfo in Structures.h 
    float fSampleRate { 44100.0f }; ///< Current sample rate in Hz.
